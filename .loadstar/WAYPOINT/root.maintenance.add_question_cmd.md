...
- TECH_SPEC:
  # TASK
  - [ ] cmd/question.go 신규 작성
  - [ ] WAYPOINT/*.md 파일 전체 스캔 로직
  - [ ] OPEN_QUESTIONS 라인 regex 추출 (`[Q(\d+)(?:\s+(DEFERRED|RESOLVED\s+[\w.-]+))?\]`)
  - [ ] 기본 출력: ADDRESS, QID, STATE, QUESTION 테이블
  - [ ] FILTER 인자 지원 (주소/키워드 부분일치)
  - [ ] `loadstar question stats` 서브옵션 — OPEN/DEFERRED/RESOLVED 집계
  - [ ] cmd/root.go에 AddCommand 등록
  - [ ] 단위 테스트 작성
  # RECURRING
